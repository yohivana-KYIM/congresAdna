        ...
        <div class="flex flex-col items-center">
          ...
          <div class="flex space-x-2">
            ...
            <button 
              ...
              class="px-4 py-2 text-sm font-semibold text-white transition-colors duration-300 ease-in-out rounded-full bg-lime-500 hover:bg-lime-600"
            >
              Réinitialiser
            </button>
          </div>
  ...
  setup() {
    ...
    const displayText = computed(() => {
      ...
      if (days + hours + minutes + seconds <= 0) {
        isClosed.value = true;  // Marque le congrès comme fermé
    ...
    const toggleCountdown = () => {
      if (isClosed.value) {
        isClosed.value = false; // Réactive le compte à rebours si le congrès est fermé
        isRunning.value = true;
        now.value = Date.now(); // Réinitialise le temps actuel
      } else {
    ...
    const resetCountdown = () => {
      ...
      targetDate.value = new Date('2025-01-23T00:00:00').getTime(); // Réinitialise la date cible
    };
