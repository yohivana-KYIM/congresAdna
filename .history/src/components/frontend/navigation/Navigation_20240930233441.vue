  ...
  <nav class="bg-[#9C7C13] text-white py-2">
    <div class="container px-4 mx-auto">
      <div class="flex items-center justify-center space-x-10">
        <!-- Accueil with dropdown -->
        <div class="relative">
          <div @click="toggleSubmenu('accueil')" class="px-2 py-1 transition-all duration-300 bg-opacity-50 rounded-lg cursor-pointer nav-item hover:bg-opacity-70">
          ...
          <div v-if="openSubmenu === 'accueil'" class="absolute z-10 w-64 py-1 mt-2 bg-white rounded-md shadow-lg bg-opacity-80 backdrop-blur-lg">
            <a v-for="(item, index) in accueilItems" :key="index" href="#" class="block px-4 py-2 text-sm text-gray-800 transition-colors duration-200 hover:bg-gray-300">
              {{ item }}
            </a>
          </div>
        </div>

        <div @click="toggleSubmenu('congres')" class="px-2 py-1 transition-all duration-300 bg-opacity-50 rounded-lg cursor-pointer nav-item hover:bg-opacity-70">
        ...
        <div @click="toggleSubmenu('participer')" class="px-2 py-1 transition-all duration-300 bg-opacity-50 rounded-lg cursor-pointer nav-item hover:bg-opacity-70">
        ...
        <div class="items-center hidden space-x-4 md:flex">
          <div @click="toggleSubmenu('mediatheque')" class="flex flex-col items-center px-2 py-1 transition-all duration-300 bg-opacity-50 rounded-lg cursor-pointer hover:bg-opacity-70">
          ...
          <div @click="toggleSubmenu('hymne')" class="flex flex-col items-center px-2 py-1 transition-all duration-300 bg-opacity-50 rounded-lg cursor-pointer hover:bg-opacity-70">
      ...
      <transition name="slide-fade">
        <div v-if="isMenuOpen" class="mt-4 md:hidden">
          <div class="mb-4 nav-item" @click="toggleSubmenu('mediatheque')">
            <div class="flex flex-col items-center px-2 py-1 transition-all duration-300 bg-opacity-50 rounded-lg cursor-pointer hover:bg-opacity-70">
              <div class="icon-circle bg-[#DAA520]">
                <img src="/images/menu/Médiathèque.svg" alt="Médiathèque" class="w-6 h-6">
              </div>
              <span class="mt-1 text-xs">Médiathèque</span>
