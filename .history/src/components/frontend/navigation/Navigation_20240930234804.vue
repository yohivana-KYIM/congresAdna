    ...
    <div class="container px-4 mx-auto">
      ...
      <transition name="slide-fade">
        <div v-if="isMenuOpen" class="mt-4 md:hidden">
          ...
          <!-- New Dropdown Select for Accueil -->
          <div class="mb-4 nav-item">
            <label class="text-xs">Accueil</label>
            <select v-model="selectedAccueil" @change="handleAccueilChange" class="mt-1 text-white bg-transparent bg-site-bg nav-select hover:bg-gray-600">
              <option value="">Sélectionnez</option>
              <option value="motChef">Mot du Chef Canton NDOGBATJECK</option>
              <option value="motPresident">Mot du Président ADNA NDOGBATJECK</option>
              <option value="presentation">Présentation du Canton NDOGBATJECK</option>
              <option value="villages">Listes des villages NDOGBATJECK</option>
              <option value="associations">Listes des Associations NDOGBATJECK</option>
              <option value="hote">L’Hôte du congrès NDOGBATJECK</option>
            </select>
          </div>
        </div>
      </transition>
...
export default {
  data() {
    return {
      ...
      openSubmenu: null,
      selectedLanguage: 'fr',
      selectedAccueil: '',  // New data property for Accueil
    };
  },
  methods: {
    toggleMenu() {
    ...
    handleAccueilChange() {
      console.log('Option sélectionnée pour Accueil:', this.selectedAccueil);
    },
  },
};
</script>
