<template>
  <section class="max-w-6xl py-10 mx-auto">
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 rounded-lg overflow-hidden">
      <article 
        v-for="post in posts" 
        :key="post.id" 
        class="overflow-hidden bg-white rounded-lg shadow-md transition duration-300 hover:bg-green-200 hover:shadow-lg"
      >
        <div class="relative group">
          <img 
            :src="post.image" 
            alt="Image de l'article" 
            class="object-contain w-full h-48 transition-transform duration-300 transform group-hover:scale-110 cursor-pointer"
          >
        </div>
        <div class="p-4">
          <h3 
