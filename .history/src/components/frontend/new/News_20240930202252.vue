  ...
  <section class="max-w-6xl py-10 mx-auto">
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
      <article 
        v-for="post in posts" 
        :key="post.id" 
        class="overflow-hidden bg-white rounded-lg shadow-md transition-transform duration-300 transform hover:scale-105"
      >
        <img 
          :src="post.image" 
          alt="Image de l'article" 
          class="object-cover w-full h-48 transition-opacity duration-300 ease-in-out hover:opacity-75"
        >
        <div class="p-4">
