<template>
  <section class="container px-4 py-12 mx-auto md:py-16 lg:py-20">
    ...
    <div class="grid grid-cols-1 gap-6 overflow-hidden rounded-lg md:grid-cols-2 lg:grid-cols-3">
      <article 
        ...
        :key="post.id" 
        class="overflow-hidden transition-all duration-300 bg-white hover:shadow-lg rounded-xl"
      >
        ...
        <div class="p-6">
          <h3 
            class="mb-4 text-2xl font-bold transition-all duration-300 transform text-primary group-hover:-translate-y-1"
          >
            {{ post.title }}
          </h3>
          <p 
            class="mb-6 text-gray-600 transition-all duration-300 transform group-hover:translate-x-1"
          >
            {{ post.excerpt }}
          </p>
          <router-link 
            :to="`/blog/${post.id}`" 
            class="inline-block px-6 py-2 text-white transition-colors duration-200 bg-blue-500 rounded-full hover:bg-blue-700"
          >
            Lire la suite
          </router-link>
        </div>
    ...
    <div class="flex justify-center mt-12 space-x-4">
      <button 
        ...
        class="flex items-center justify-center w-10 h-10 text-gray-800 transition-all duration-200 transform bg-transparent rounded-full hover:bg-gray-100"
      >
      ...
      <div class="flex items-center">
        <span 
          ...
          :key="index" 
          @click="goToView(page)" 
          class="px-4 py-2 mx-2 text-sm text-gray-600 transition-colors duration-200 cursor-pointer hover:text-primary"
        >
          {{ page }}
        </span>
      </div>
