<template>
  <div class="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-900 to-yellow-700 relative overflow-hidden">
    ...
    <!-- Conteneur principal -->
    <div class="relative z-10 flex flex-col items-center p-8 bg-black bg-opacity-50 rounded-2xl shadow-2xl backdrop-blur-md transition-all duration-500 hover:bg-opacity-60">
      <h1 class="mb-6 text-5xl font-extrabold text-white drop-shadow-lg animate-title">
        Hymne National du Cameroun
      </h1>

      <!-- Paroles de l'hymne -->
      <div class="max-w-2xl mx-auto bg-yellow-900 bg-opacity-30 rounded-xl shadow-inner p-6 backdrop-blur-sm overflow-y-auto max-h-96 transition-all duration-500 hover:bg-opacity-40">
        <p
          v-for="(line, index) in hymnLyrics"
          :key="index"
          class="text-2xl text-white leading-relaxed my-2 opacity-90 animate-fadeIn"
      ...
      <div class="mt-10 flex items-center space-x-4">
        <button
          ...
          class="group bg-yellow-600 hover:bg-yellow-500 text-white font-bold py-3 px-6 rounded-full transition-all duration-300 transform hover:scale-105 active:scale-95 shadow-lg focus:outline-none flex items-center justify-center"
        >
